...
export function DataTable<TData, TValue>({
  ...
  const exportRows = table
    .getFilteredRowModel()
    .rows.map((row) => row.original as UserRow)
  ...
  const exportToCSV = () => {
    ...
    const csvData = exportRows.map((user) => ({
    ...
    anchor.href = url
    anchor.download = `users-${new Date().toISOString().split('T')[0]}.csv`
    anchor.click()
  ...
  const exportToJSON = () => {
    ...
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `users-${new Date().toISOString().split('T')[0]}.json`
    anchor.click()
    URL.revokeObjectURL(url)
